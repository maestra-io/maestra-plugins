...
Read **once, at the moment the report is begun**: the one home of what the report may say, in what
words, what each section owes, and which forms it takes.
...
included. The reader is the person who will present these numbers to stakeholders, or the
stakeholders themselves, so every figure has to survive being quoted, and **nothing of ours reaches
the surface**: no template
...
**Translate each term once and hold it to the last line**: the fixed vocabulary is
`references/<locale>/terminology.md` (`en-US`, `ru-RU`), and what each of this skill's terms becomes
on the surface is `findings-and-failures.md`, "Our words and the reader's".

## Money: one pair, said in the product's words

Every money figure is aggregated **by the date the message was sent** — `metric.revenue`, which is
`flow_report`'s `Revenue` and which the product's scenario report labels *Total Revenue (First touch
within data range)*; the wording per locale is in `references/<locale>/terminology.md`. Name it that
way once, in the header, and say that it will not match `metric.revenueByOrderDate` (*Total
revenue*), which counts the same money on the date of the order, so the same order lands in a
different month. Never print both to settle it.

**Say which screen a label came from whenever one is printed.** The wording the reader has in front
of them depends on the report they opened — the mailings report and the scenario report label the
same quantity differently — and a label printed with no screen beside it is what makes a reader
call the figure wrong. One label is shared outright: *Total revenue* is the money attributed on the
order date, while the project's **total revenue for the period** — everything the project earned,
attributed or not — carries almost the same label, and the two are never the same number. Neither
is printed beside the other without that sentence.
...
the chat, `<a href>` on a page. The base comes from a link the user pasted, or from the address the
user gives — on this platform `https://<system name>.maestra.io`, the system name being the `tenant`
every call takes — and the report says once which base the links are built from.
**No bare identifier on any surface**, in any form. Two exceptions, both said in the report: no base
could be built; or the user explicitly said the page will be **shown on a screen** rather than
opened. The chat answer never carries one whatever the answer.

## Written for both readers

The report is forwarded to the stakeholders as it stands. Every step and every probe is worded so both
...
1. **Header** — project; period; the goal by display name; the basis of the verdicts and whether it
   was chosen or defaulted; the report form and whether it was chosen or defaulted; the one
   sentence that the freshness of the reporting data could not be measured, because no call reports
   it; what could not be reached, as a capability ("the email health benchmark was not available",
   "per-customer figures are not populated on this project"); coverage ("classified N of M scenarios this access sees,
   plus K in the reporting data and not in the list") — M is what this connection lists, and the
   line says so; the confidence split (how many confident
   verdicts, how many "can't judge", and the two thresholds); the consistency check — the project's
   own line against the sum of the scenario rows, and whether the breakdown was capped
   (`metrics-map.md`, "Retired handles"); the money sentence above.
...
4. **The scenarios with a problem — in full.** One block each, **up to seven of them, as many as
   have a figure and a construction reading behind them** — never padded to seven, never cut to a
   round number, and one or two where that is all the project has. Ordered by the money at stake; volume or reach where there is none. Five parts,
   below.
...
6. **What works — the top performers.** Two or three scenarios by conversion to order (the
   platform's own rate) or revenue per message on volume above the floor, each with its figure, its comparison, and **what in its
...
- **The full page** — offered at the end, built only on request from `<locale>/html-report-example.html`;
  same verdicts, figures, scenarios and sections as the chat answer, bar that file's shorter table.
- **The proposal deck and its presenter notes** — built when the report form chosen at `SKILL.md § Inputs`
  is the deck, from `<locale>/html-proposal-deck-example.html`. The deck carries a **subset** of the full
...
**The deck is the report taken apart into proposals, and it is ready to present as it stands.**
Nothing on it is addressed to the presenter. It has two readers, the presenter who runs the meeting
and the stakeholders who receive the file afterwards, and one voice — the proposal.

**Slides: one overview plus up to seven proposals** — as many as the full report took through in
full, and a deck of two slides is a correct deck on a project with two such findings. The overview lists the proposals with their
...
1. **Context line** — the proposal number, the scenario(s) by name as links, and an urgency label on
   the right from a closed set of three: `urgency.now` / `urgency.fix` / `urgency.idea`, whose
   wording is `references/<locale>/terminology.md` ("Decide today" / "Fix" / "Idea" in English),
   fixed once in the log for a language with no variant. **Assigned by rule** (`SKILL.md` step 8): a
   breached hygiene threshold or the attribution finding → `urgency.now`; an established cause with
   a fix → `urgency.fix`; a hypothesis with a probe → `urgency.idea`. Labels are not verdicts:
   verdicts stay in the full report; a label says how soon, not what state.
...
6. **Three disclosures, in this order:**
   - **the evidence** — headed by what it shows: `deck.heading.whereLost`,
     `deck.heading.whatChecked`, `deck.heading.whatThisDoesNotExplain`. The per-block funnel with
     its window, the month-by-month series, the per-message split, the year-ago comparison go here;
   - **`deck.heading.inTheScenario`** — part 3 of the full block, shortened: the blocks by name, the
     settings as set, the counts with their window;
   - **`deck.heading.whatToDo`** — the steps, verbatim from the full report's "What to do", with
     their addressees.

**A small inline chart (SVG or CSS bars, no library) is welcome** where a series carries the slide,
with the same window and origin under it, drawn only from figures already in the full report.

**Identifiers in the deck**: never, unless the user asked.

**What the deck does not carry**: the overview table, the four verdict words, the did-not-send
counter, the provability thresholds and their numbers, the coverage counts, the consistency check,
`## What we do not know` as a section, weak verdicts, below-floor scenarios, anything of our
machinery. `## What we do not know` becomes one footnote line under the navigation: the data
windows, the goal, the money sentence.

### The presenter notes — a separate file, never in the deck

Everything the presenter needs and the stakeholders should not read goes to **a separate Markdown
file beside the deck**, one section per proposal slide, named for the deck with a `-notes` suffix.
It carries, per slide:

- **how to say it** — one paragraph of 60–120 words the presenter can read aloud in under a minute, first
...
- **what not to say** — where a wording would outrun the data (a coincidence in time worded as a
  ...
  where an objection is right, and what to answer then.

**The notes are handed over with one sentence**: that they are for whoever presents the deck and are
not part of what the stakeholders receive.
